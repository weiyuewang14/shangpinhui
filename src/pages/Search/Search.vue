            ...
            <div class="py-container">
                ...
                <div class="bread">
                    ...
                    <ul class="fl sui-tag">
                        ...
                        <!--  -->
                        <li class="with-x" v-for="(attrValue,index) in searchParams.props" :key="Search">
                            {{attrValue.split(":")[1]}}<i @click="removeAttr(Search)">×</i></li>
                    </ul>
                </div>
                ...
                <div class="details clearfix">
                    ...
                    <Pagination :pageNo="searchParams.pageNo" :pageSize="searchParams.pageSize" :total="total"
                                :continuous="5" @getPageNo="getPageNo"/>
                </div>
            </div>
        </div>
    </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            //向服务器发请求获取search模块数据（根据参数不同获取不同的数据）
            getData() {
            ...
            removeCategoryName() {
                //置空给服务器的参数
                //带给服务器的参数可以为空，当设置为undefined时，就不携带此参数给服务器
            ...
            //删除售卖属性
            removeAttr(index) {
                this.searchParams.props.splice(Search, 1);
                //再次发请求
                this.getData();
            },
            //排序的操作,默认为降序
            changeOrder(flag) {//flag：标记
                let originOrder = this.searchParams.order;
                let originFlag = originOrder.split(":")[0]; //是哪个：综合|价格  1|2
                let originSort = originOrder.split(":")[1]; //排序：降序（desc）|升序（asc） desc|asc
                //一个新的order属性
                let newOrder = "";
                if (flag === originFlag) {//如果初始点击的是综合|价格，不需要更改名称，需要修改排序方式
                    newOrder = `${originFlag}:${originSort === "desc" ? "asc" : "desc"}`;
                } else {//如果不是当前的名称（综合|价格），即点击的不是同一个按钮，则需要更改名称，而且设置默认值为降序
                    newOrder = `${flag}:${"desc"}`;
                }
